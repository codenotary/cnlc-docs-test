<template>
    <section class="logo-wrapper">
        <div v-if="text && !icon">
            <img class="logo" :src="getAsset" :class="getLogoSize" :alt="getAlt" :title="getTitle">
            <span v-if="text === 'default'" class="text--logo" :class="[getTextSize, getTextColor]">
                {{ getText }}
            </span>
            <span v-else class="text--logo" :class="[getTextSize, getTextColor]">
                {{ text }}
            </span>
        </div>
        <div v-else>
...
export default {
    ...
    computed: {
        getText () {
            return `Ledger Compliance® v${process.env.V_VERSION}`;
        },
        getAlt () {
